...
public partial class Home : AutoScrollComponentBase
{
    ...
    private static readonly List<(string Major, string? Minor)> s_whyContent1 = new()
    {
        ("部署安装服务", null),
        ("线上故障修复", null),
        ("服务巡检", null),
    ...
    private static readonly List<(string Major, string? Minor)> s_whyContent2 = new()
    {
        ("基础架构类", "如上云、架构升级、DevOps集成等"),
        ("外包服务类", "如应用现代化重构、物联网、电商项目等"),
    };

    private static readonly List<(string Major, string? Minor)> s_whyContent3 = new()
    {
        ("定制化培训服务", null),
